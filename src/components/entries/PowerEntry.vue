  ...
  <button class="list-group-item list-group-item-action flex-column align-items-start"
    ...
    <div>
      <div class="d-flex justify-content-between" style="margin-right: 50px;">
        ...
        <small v-if="tractive > 0 ">Tractive: {{ tractive }}kN</small>
        <small>{{ power }}kW</small>
      </div>
...
const power = ref(props.item[1]);
let tractive = 0;
if(props.item.length > 2){
  tractive = ref(props.item[2]);
}
...
console.log("power", props.item, hasSteamLink.value)
